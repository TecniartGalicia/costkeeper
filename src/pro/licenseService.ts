...
const STATE_HAS_LICENSE = 'costkeeper.hasLicense';
 ...
 * honest upsell and returns false. Removing anything CostKeeper added is always free (see features.ts).
 */
export async function ensurePro(context: vscode.ExtensionContext, feature: string): Promise<boolean> {
  ...
  switch (d.reason) {
    case 'expired':
      msg = l10n.t('Your CostKeeper Pro licence has expired.');
      break;
    case 'revoked':
    case 'invalid':
      msg = l10n.t('Your CostKeeper Pro licence key is not valid any more.');
      break;
    case 'activation-removed':
      msg = l10n.t("This computer's activation of your CostKeeper Pro key was removed (from the Polar customer portal or by activating elsewhere). Enter the key again to re-activate.");
      break;
    case 'grace-expired':
      msg = l10n.t('CostKeeper Pro could not be re-validated for more than 14 days (no network). Connect once and try again.');
      break;
    case 'network':
      msg = l10n.t('CostKeeper Pro licence could not be validated (no network). Try again when you are online.');
      break;
    default:
      msg = l10n.t('"{0}" is part of CostKeeper Pro. The free version keeps the whole index, the panel and the quota view forever; Pro adds client tagging, CSV export, budgets, quota forecast and unlimited history.', feature);
  }
...
export async function openCheckout(): Promise<void> {
  if (!POLAR_CHECKOUT_URL) {
    void vscode.window.showInformationMessage(l10n.t('CostKeeper Pro is not on sale yet. Watch the repository for the announcement.'));
    return;
  }
...
export async function activateLicenseCommand(context: vscode.ExtensionContext): Promise<void> {
  ...
  const key = await vscode.window.showInputBox({
    title: l10n.t('CostKeeper Pro — enter your licence key'),
  ...
  const r = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: l10n.t('Activating CostKeeper Pro…'), cancellable: true }, async (_p, token) => {
    ...
    if (!res.ok && res.kind === 'limit' && /limit/i.test(res.message) && hadSlot && previous.key === trimmed) {
      ...
      if (!freed) {
        ...
        void vscode.window.showWarningMessage(l10n.t("CostKeeper Pro is active, but this computer's previous activation could not be released; it may still count against the activation limit. You can free it from your Polar customer portal."));
      }
    }
    return res;
  });
  ...
  if (!r.ok) {
    ...
    void vscode.window.showErrorMessage(l10n.t('Could not activate CostKeeper Pro. {0}', why) + note);
    return;
  }
  ...
  else void vscode.window.showInformationMessage(l10n.t('CostKeeper Pro activated on this computer. Thank you!'));
}

export async function deactivateLicenseCommand(context: vscode.ExtensionContext): Promise<void> {
  const state = await loadState(context);
  if (!state.key) {
    void vscode.window.showInformationMessage(l10n.t('No CostKeeper Pro licence is stored on this computer.'));
    return;
  }
  const yes = l10n.t('Deactivate');
  const pick = await vscode.window.showWarningMessage(l10n.t('Deactivate CostKeeper Pro on this computer? The activation slot is freed so you can use the key elsewhere.'), { modal: true }, yes);
  if (pick !== yes) return;
  if (state.activationId && polarConfigured()) {
    const ok = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: l10n.t('Deactivating CostKeeper Pro…') }, () => polarDeactivate(fetchImpl, cfg(), state.key!, state.activationId!));
  ...
  invalidateProCache();
  void vscode.window.showInformationMessage(l10n.t('CostKeeper Pro deactivated on this computer.'));
}
...
export async function licenseStatusCommand(context: vscode.ExtensionContext): Promise<void> {
  ...
  const d = await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: l10n.t('Checking CostKeeper Pro licence…'), cancellable: true },
    (_p, token) => proStatus(context, true, token),
  );
  const state = await loadState(context);
  const lines = [
    d.pro ? l10n.t('CostKeeper Pro: active ({0})', reasonText(d)) : l10n.t('CostKeeper Pro: not active ({0})', reasonText(d)),
